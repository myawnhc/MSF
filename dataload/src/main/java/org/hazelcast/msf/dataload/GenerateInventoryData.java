...
@Deprecated
public class GenerateInventoryData {
    ...
    public int getItemCount() { return itemDAO.getItemCount(); }
    public int getInventoryCount() { return inventoryDAO.getInventoryRecordCount(); }

    public boolean hasItemData() {
        Item i = itemDAO.findByKey("10101");
        return i != null;
    }
    public boolean hasInventoryData() {
        InventoryKey ik = new InventoryKey("10101", "W1");
        Inventory i = inventoryDAO.findByKey(ik);
        return i != null;
    }
    ...
    public void OLDgenerateItems(int count) {
        //ItemDAO itemDAO = new ItemDAO(controller);
        Map<String, Item> items = new HashMap<>(count);
        int initialItemNumber = 10101;
        for (int i=0; i<count; i++) {
            Item item = new Item();
            String itemNumber = ""+initialItemNumber++;
...
//    }
    ...
    public void generateInventory(int items, int locations, int warehouses) {
        ...
        for (int i=0; i<items; i++) {
            ...
            Map<InventoryKey, Inventory> invForItem = new HashMap<>(locations);
            for (int l=0; l<locations; l++) {
                ...
                invForItem.put(inv.getKey(), inv);
            }
            inventoryDAO.insertAll(invForItem);
        }
