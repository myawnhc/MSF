...
package com.hazelcast.msf.dashboard;/*
 *  Copyright 2018-2021 Hazelcast, Inc
 *
 *  Licensed under the Apache License, Version 2.0 (the "License");
 *  you may not use this file except in compliance with the License.
...
public class Graphite {
    ...
    public Graphite(String host){
        ...
        try {
            ...
            log.info("com.hazelcast.msf.dashboard.Graphite socket: " + graphiteSocket.getInetAddress());
        } catch (IOException e) {
            log.severe("Unable to create Graphite socket at " + host + ":" + PORT);
            e.printStackTrace();
            System.exit(-1);
        }
    ...
    private void debug(String name) {
    	if (count % 1000 == 0) {
    		log.info("com.hazelcast.msf.dashboard.Graphite: write stats " + count + "'" + name + "'");
    	}
    	count++;
    }
}
